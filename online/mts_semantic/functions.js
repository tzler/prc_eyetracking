...
function generate_stimuli(typical_category, oddity_category, i_variation, stimulus_path){
  ...
  image0 = stimulus_path + stimulus_info.typical.index[0] + '.jpeg'
  image1 = stimulus_path + stimulus_info.typical.index[1] + '.jpeg'
  image2 = stimulus_path + stimulus_info.oddity.index + '.jpeg'

  if (params['experiment_type'] == 'sequential_mts') {
    
    // decide the correct side 
    correct_side = shuffle(['left', 'right'])[0]
    // set image to be presented on the left 
    left  = [image2, image1][ 1 * (correct_side == 'left' ) ]
    // set image to be presented on the right
    right = [image2, image1][ 1 * (correct_side == 'right') ]
    // order is screen1, screen2_left, screen2_right 
    stimuli = [image0, left, right]
    // 
    correct_response = [0, 1][ 1 * (correct_side=='left') ]
  }

  if (params['experiment_type'] == 'concurrent') {

    // store images in a list whose order will be used to generate display 
    var stimuli = [image0, image1]
    // randomly decide which index the oddity should be placed into 
    ...
    stimuli.splice(random_location_within_array, 0, image2);
    // store the index of the random location as the correct location 
    correct_response = random_location_within_array + 1
  }

  // data to return  
  trial_structure_info = {
                          stimuli:stimuli, // both 
                          image_path: params.image_path, //  new
                          answer: correct_response, /// compatibility with new concurrent scripts 
                          object0: typical_identity,  /// compatibility with new concurrent scripts 
                          object1: oddity_identity, /// compatibility with new concurrent scripts 
                          trial_comparison: category_type, /// compatibility with new concurrent scripts 
                          comparison: category_type, /// compatibility with old concurrent scripts 
                          correct_response: correct_response, // compatibility with old concurrent scripts 
                          typical_category:typical_category, // old 
                          oddity_category: oddity_category,// old
                          typical_indices: stimulus_info.typical.index, // old 
                          oddity_index: stimulus_info.oddity.index, // old 
                          category_type: category_type, // old 
                          typical_identity:  typical_identity, // old 
                          oddity_identity: oddity_identity, // old 
                          variation_level: i_variation, // old
                         }
